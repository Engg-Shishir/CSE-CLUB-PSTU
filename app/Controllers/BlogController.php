...
class BlogController
{
    ...
    public function blogByCategory($category,$number=1){
        $user = new User();
        ...
        $sql = "SELECT  ud.name AS uname,
            ...
            INNER JOIN user_details AS ud ON ud.user_id=b.user_id
            INNER JOIN blog_categories AS bc ON bc.category_id=b.category_id 
            WHERE bc.category_slug = ? AND b.blog_status = 1
            ORDER BY blog_id DESC LIMIT ". $page_first_result . ','. 4;

        $stmt = $user->execute($sql,[$category]);
        $blog = $stmt->fetchAll();



        $sql = "SELECT bc.category_slug AS uslug,
                       b.Blog_id FROM blogs AS b
            INNER JOIN blog_categories AS bc ON bc.category_id=b.category_id 
            WHERE bc.category_slug = ? AND b.blog_status = 1
            ORDER BY blog_id DESC";
        ...
        return view("Frontend/Blog/CategoryBlog.php", compact("compact"));
    }
